...
Educational GUI application for Multilayer Neural Networks (MLN). Java 26, Gradle 9.4.1, SWT/JFace UI.
...
src/main/java/edu/yaprnn/
  ...
  gui/          SWT/JFace UI: views (shells, composites), model (tree nodes), services
  ...
  support/      AWT image utilities, SWT helpers, Jackson/Random configurers
...
- **SWT/JFace**: native OS widgets, JFace viewers (TreeViewer, ComboViewer), SWT Shell/Composite
...
- Tree model: `NetworksTreeModel` with JFace `TreeViewer`, lazy-loading node suppliers; `DefaultNode.applyValueChange()` for polymorphic edits
